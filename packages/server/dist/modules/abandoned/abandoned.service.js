...
function buildingsForLevel(level) {
    ...
    const rand = (max) => Math.max(1, Math.floor(Math.random() * max) + 1);
    return [
...
class AbandonedVillageService {
    ...
    async resetAfterRaid(villageId) {
        const village = await this.prisma.village.findUnique({
            where: { id: villageId },
            include: { buildings: true },
        });
        if (!village || !village.isAbandoned)
            return;
...
exports.AbandonedVillageService = AbandonedVillageService;
