...
exports.initAttackWorker = initAttackWorker;
...
// ─────────────────────────────────────────────────────────────
function safeEmit(fastify, room, event, data) {
    try {
...
function initAttackWorker(fastify) {
    const connection = new ioredis_1.default(process.env.REDIS_URL || 'redis://localhost:6379', {
        maxRetriesPerRequest: null,
    });
    const worker = new bullmq_1.Worker('attacks', async (job) => {
        ...
        if (job.data.returning === true) {
            ...
            const survivors = job.data.units;
        ...
        const shieldUntil = defenderVillage.shieldUntil;
        ...
        const defGroups = isAbandonedTarget
            ? []
            : (defenderVillage.troops ?? [])
                .filter((t) => t.count > 0)
                .map((t) => {
                try {
        ...
        const wallLevel = (defenderVillage.buildings ?? [])
            .find((b) => b.buildingId === 'wall')?.level ?? 0;
        const result = (0, shared_1.resolveBattle)(attackerGroups, defGroups, {
            wallLevel,
            attackerPoints: atkPlayer?.totalPoints ?? 1,
            defenderPoints: defPlayer?.totalPoints ?? 1,
        });
        const loot = result.attackerWon
            ? (0, shared_1.calculateLoot)({ wood: defenderVillage.wood, stone: defenderVillage.stone, iron: defenderVillage.iron }, result.lootCapacity)
            : { wood: 0, stone: 0, iron: 0 };
        const pointsExchanged = isAbandonedTarget
            ? 0
            : (0, shared_1.calculatePointsExchanged)(result.defenderLosses, defGroups);
        const survivors = {};
        for (const [unitType, count] of Object.entries(units)) {
            const survived = Math.max(0, count - (result.attackerLosses[unitType] ?? 0));
            if (survived > 0)
                survivors[unitType] = survived;
        }
        ...
        await fastify.prisma.$transaction(async (tx) => {
            ...
            await tx.attackReport.create({
                data: {
                    ...
                    defenderUnitsBefore,
                    defenderUnitsAfter: Object.fromEntries(defGroups.map(u => [u.unitType, Math.max(0, u.count - (result.defenderLosses[u.unitType] ?? 0))])),
        ...
        if (isAbandonedTarget && result.attackerWon) {
            await fastify.abandonedService.resetAfterRaid(defenderVillageId);
        }
